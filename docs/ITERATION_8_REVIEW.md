...
The first reformulation moved the gate into the stop outputs of a top-level zoom `interpolate` —
which the red-team then caught as a *second* defect: interpolation blends **between** gated
outputs, so a river was already ~30% opaque below its own reveal zoom. The gate the comment
promised still was not the gate that shipped.

The shipped form is a top-level `step` whose bucket boundaries are the **distinct `reveal_zoom`
values the pipeline emits** (0 / 2.6 / 3.4 / 4.2 / 5.0), so the constant compared against
`reveal_zoom` inside each bucket is exact rather than approximate. Generated by
`buildRiverOpacity()` so the style cannot drift from the data, and `rivers.test.ts` fails if the
data grows a threshold the buckets do not cover, if a river reveals early or late, or if
`["zoom"]` ever reappears nested inside a `case`. No score or facet is affected — rivers are
context geography and are never scored.

The layer has now produced two defects from the same root cause: an invalid-but-non-throwing
expression, then a valid one that did not do what its comment said. Both are pinned by tests.
...
| Frontend tests | ✅ **96** (was 33, was 10 pre-iteration) |
...
| Clean git tree | ✅ |
| Merged, pushed, deployed | ✅ `4425ef7` live on Vercel |
| Live production verification | ✅ §14 |

---

## 14. Live production verification

Merged to `main` and deployed to Vercel. `origin/main` carried an automated
`dataset-refresh[bot]` rebuild for the same day, which conflicted in the data files; resolved by
regenerating from source (the database is a build artifact), with headline agreement confirmed.

Verified on **https://russian-energy-dashboard.vercel.app**:

| Check | Result |
|---|---|
| `as_of` / ESDI | **2026-08-30 / 17.86**, `build_time` same day → current-date build, not frozen |
| Recovery-evidence fix live | 30d shows **2** restorations, 90d shows **6** — where the old source could only ever show 1 |
| Window honesty | "Actual comparison span" 8 / 36 / 92 days at 7 / 30 / 90 |
| Icons, precision variants, legend, filter glyphs | render; dashed and stacked variants distinguishable |
| Hover / click / dossier sub-card | public attributes only, co-located assets named |
| 30d + 90d change choropleth | diverging ramp, ticks, "index fell / rose", decay caveat |
| Activity halos + legend key | present |
| What changed 7/30/90 | scoped, separate measures |
| Comparison tray + sparklines + search | working; search shows class **and** region, "LNG Terminal" cased correctly |
| Rivers | geometry renders, "Volga" label appears, reveal gate correct, no console errors |
| Gas / oil network, grid, borders, sea + country names | render |
| Crimea | violet banner, sovereignty + de-facto control, "· occupied", separate styling |
| Timeline scrubber, all 7 analytical tabs | working |
| Shareable URL | full round-trip incl. Crimea selection, comparison set, camera |
| Malformed URL | rendered, every invalid value defaulted, phantom region dropped, **URL self-sanitised to empty**, no crash |
| Hard reload | clean |
| Console | **zero messages** of any kind |
| Network | **every request same-origin** + one same-origin `blob:` worker — zero third-party runtime dependencies |

---

## 15. Limitations (aggressive)
...
- **Headless verification friction.** The in-app browser pane pauses MapLibre's render loop when
  ...
  they should be.
- **The exposure choropleth still cannot follow the type filter.** It is disclosed rather than
  fixed, because narrowing ESDI to a class subset would redefine the measure — a scoring change.
  If per-class exposure is genuinely wanted, it needs a pipeline-side per-class index and its own
  denominators, which is an iteration of its own.
- **Icon shape legibility has a floor.** Sizes were raised at the home view and two silhouettes
  redrawn, but at continental zoom a 10 px glyph still leans on colour. The remaining pairs are
  separated by hue and by the fact that shape resolves on zoom-in; a genuinely
  shape-first-at-all-zooms set would mean fewer, coarser classes on the map.
- **Substation colour now sits in the same family as transmission line.** Deliberate — they are
  one subsystem and never share a mark — but it does mean the grid layers read as a single visual
  group, which is a choice someone may want to revisit.
- **The daily refresh bot will rewrite the data files.** Anything that depends on the exact
  committed payload (including screenshots in docs) goes stale within a day; the headline numbers
  quoted here are the 2026-08-30 build.
